 ...
 * genericity class T invoke to Bean class and new instance. parser block with line
...
public class BeansPrep<T extends Bean> {
    ...
    private void Setter(T bean, String[][] ValueType, int no, byte[] Block, int beanStart, int beanEnd)
            throws Exception {
        byte[] tempValue = new byte[beanEnd + 1 - beanStart];
        System.arraycopy(Block, beanStart, tempValue, 0, beanEnd + 1 - beanStart);
        Class<?> tempType = Class.forName(TempClassUtil.SupplyClassName(ValueType[no][1]));
        BeanClass.getMethod("set" + ValueType[no][0], tempType).invoke(bean, tempType,
    ...
    private void InitBeanStruct() {
        InitProp();
        String[] Values = props.getProperty(BeanClass.getName()).split(",");
